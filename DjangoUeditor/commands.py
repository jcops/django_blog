...
class UEditorEventHandler(object):
    ...
    def render(self, editorID):
        jscode = """
            %(DjangoUeditor)s.addListener('%(event)s', function () {
                %(event_code)s
        });"""
        event_codes = []
        ...
        for event in events:
            try:
                ...
                if event_code:
                    event_code = event_code % {"DjangoUeditor": editorID}
                    event_codes.append(jscode % {"DjangoUeditor": editorID, "event": event[
...
class UEditorCommand(object):
    ...
    def render_command(self):
        ...
        if len(cmds) > 0:
            return u"""
            DjangoUeditor.registerCommand(uiName, {
                    %s
                });
            """ % ",".join(cmds)
        else:
    ...
    def render(self, editorID):
        return u"""
        UE.registerUI("%(uiName)s", function(DjangoUeditor, uiName) {
            %(registerCommand)s
            %(uiObject)s
        },%(index)s,"%(editor)s");
        """ % {
...
class UEditorButtonCommand(UEditorCommand):
    ...
    def onClick(self):
        ...
        return """
            DjangoUeditor.execCommand(uiName);
        """
